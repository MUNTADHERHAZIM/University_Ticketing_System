...
from .forms import CreateTicketForm, CloseTicketForm, AcknowledgeTicketForm, CommentForm
...
def dashboard(request):
    ...
    context = None  # Force refresh for debugging
    
    if not context:
        logger.info(f'Dashboard cache miss for user {user.id}')
        context = {'user': user}
        
        if user.role == 'president' or user.role == 'admin':
            # لوحة رئيس الجامعة - عرض كل شيء
            now = timezone.now()
            ...
            dept_filter = Q(department=user.department) if user.department else Q()
            
            context.update({
                'my_dept_tickets': Ticket.objects.filter(dept_filter).count(),
                'my_dept_pending': Ticket.objects.filter(dept_filter, status__in=['new', 'pending_ack', 'in_progress']).count(),
                'my_dept_violated': Ticket.objects.filter(dept_filter, status='violated').count(),
                'recent_tickets': Ticket.objects.select_related(
                    'department', 'assigned_to', 'created_by'
                ).filter(dept_filter).order_by('-created_at')[:10],
            })
            
            # متوسط وقت الحل
            resolved_tickets = Ticket.objects.filter(dept_filter, status__in=['resolved', 'closed'])
            ...
            # لوحة الموظف
            context.update({
                'my_tickets': Ticket.objects.filter(assigned_to=user).count(),
                ...
                ).filter(assigned_to=user).order_by('-created_at')[:10],
            })
            
            # معدل الإنجاز
            total = Ticket.objects.filter(assigned_to=user).count()
            resolved = Ticket.objects.filter(assigned_to=user, status__in=['resolved', 'closed']).count()
...
def acknowledge_tickets(request):
    """
    صفحة تأكيد استلام الطلبات - إجبارية
    """
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'تم تأكيد استلام {acknowledged_tickets.count()} طلب')
...
def create_ticket(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            ticket.save()
            form.save_m2m()  # حفظ ManyToMany relationships (departments)
...
def close_ticket(request, pk):
    ...
    # التحقق من الصلاحيات
    if ticket.assigned_to != request.user and not request.user.is_head:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            TicketAction.objects.create(
                ...
                action_type='closed',
                user=request.user,
                notes=form.cleaned_data['close_notes']
            )
...
def ticket_list(request):
    ...
    if request.user.role == 'employee':
        # الموظف يرى التذاكر المسندة إليه والتذاكر التي أنشأها
        tickets = tickets.filter(
            Q(assigned_to=request.user) | 
            Q(created_by=request.user)
        )
    elif request.user.role in ['head', 'dean']:
        tickets = tickets.filter(department=request.user.department)
...
def reports_dashboard(request):
    """
    لوحة التقارير والإحصائيات - "لوحة الفضح"
    """
...
def export_report(request):
    ...
    for ticket in tickets:
        writer.writerow([
            ...
            smart_str(ticket.department.name),
...
def export_ticket_pdf(request, pk):
    ...
    elements.append(Paragraph(format_arabic(f"رقم: {ticket.id}"), normal_style))
    ...
    info_table = Table(info_data, colWidths=[4.5*inch, 1.5*inch])
    info_table.setStyle(TableStyle([
        ('BACKGROUND', (1, 0), (1, -1), colors.HexColor('#f0f0f0')),  # العمود الأيمن
        ('ALIGN', (0, 0), (-1, -1), 'RIGHT'),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
    ...
    dates_data = [
        [Paragraph(ticket.created_at.strftime('%Y-%m-%d %H:%M'), normal_style), Paragraph(format_arabic('تاريخ الإنشاء:'), normal_style)],
        [Paragraph(ticket.sla_deadline.strftime('%Y-%m-%d %H:%M'), normal_style), Paragraph(format_arabic('الموعد النهائي:'), normal_style)],
    ]
    
    if ticket.resolved_at:
        dates_data.append([
            Paragraph(ticket.resolved_at.strftime('%Y-%m-%d %H:%M'), normal_style),
            Paragraph(format_arabic('تاريخ الحل:'), normal_style)
        ])
    
    dates_table = Table(dates_data, colWidths=[4.5*inch, 1.5*inch])
    ...
    if actions.exists():
        ...
        for action in actions:
            actions_data.append([
                Paragraph(action.created_at.strftime('%Y-%m-%d %H:%M'), normal_style),
